    ...
    @if($products->count() > 0)
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            @foreach($products as $product)
                <a href="{{ route('products.show', $product) }}" class="bg-white rounded-xl shadow-sm border hover:shadow-md transition group">
                    <div class="relative">                        @if($product->image)
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->nama_produk }}" class="w-full h-48 object-cover rounded-t-xl">
                        @else
                    ...
                    <div class="p-4">
                        <h3 class="font-semibold group-hover:text-[#0D9488] transition">{{ $product->nama_produk }}</h3>
                        <div class="mt-2">
                            <span class="text-lg font-bold" style="color: #0D9488;">Rp {{ number_format($product->effective_price, 0, ',', '.') }}</span>
                        </div>
                        <p class="text-sm text-stone-500 mt-1">Varian: {{ $product->productVariants->count() }}</p>
                    </div>
                </a>
            @endforeach
        </div>
