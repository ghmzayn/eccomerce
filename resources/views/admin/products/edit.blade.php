        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4"
                    class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#0D9488] focus:border-[#0D9488] outline-none">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <div class="flex items-center space-x-2">
                    <input type="checkbox" id="is_promo" name="is_promo" value="1" {{ old('is_promo', $product->is_promo) ? 'checked' : '' }}
                        class="rounded focus:ring-[#0D9488]">
                    <label for="is_promo" class="text-sm font-medium text-stone-700">Aktifkan Promo</label>
                </div>
            </div>

            <div class="mb-4" id="promo_price_container" style="{{ old('is_promo', $product->is_promo) ? '' : 'display:none' }}">
                <label for="promo_price" class="block text-sm font-medium text-stone-700 mb-1">Harga Promo</label>
                <input type="number" id="promo_price" name="promo_price" value="{{ old('promo_price', $product->promo_price) }}" step="0.01"
